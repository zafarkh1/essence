...
const Cards = ({
  ...
  return (
    <div className="relative lg:max-w-[420px] aspect-[4/5] rounded-xl overflow-hidden group cursor-pointer">
      ...
      {/* Text Container */}
      <div className="absolute bottom-0 left-0 w-full px-4 pb-4 lg:opacity-0 opacity-100 group-hover:opacity-100 transition-opacity duration-300 font-bold text-white mb-auto">
        <h5 className="lg:text-2xl text-xl lg:mb-4 mb-3">
          {`${first_name}  ${last_name}`}
        </h5>
        <p className="lg:text-base text-base mb-1">{founder}</p>
        <p className="lg:text-sm text-sm pr-5 line-clamp-2">{description}</p>
      </div>
    </div>
  );
};
